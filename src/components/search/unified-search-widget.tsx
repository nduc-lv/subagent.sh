...
interface UnifiedSearchWidgetProps {
  variant?: 'hero' | 'page' | 'compact' | 'minimal';
...
export function UnifiedSearchWidget({
  ...
  const query = externalSearchValue !== undefined ? externalSearchValue : internalQuery;
  ...
  const handleSearch = (searchQuery?: string) => {
    ...
    if (finalQuery.trim()) {
      if (onSearch) {
        ...
        router.push(`/agents?q=${encodeURIComponent(finalQuery)}`);
      }
    }
  };
  ...
  const handleCategoryClick = (categoryId: string) => {
    router.push(`/agents?category=${categoryId}`);
  };
    ...
    return (
      <div className={cn("w-full max-w-4xl mx-auto text-center space-y-8", className)}>
        ...
        <motion.div
          ...
          className="space-y-4"
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-terminal-green via-blue-500 to-purple-500 bg-clip-text text-transparent leading-tight sm:leading-relaxed min-h-[1.2em]">
        ...
        <motion.div
          ...
          className="flex flex-wrap justify-center items-center gap-4 sm:gap-6 md:gap-8 text-xs sm:text-sm text-muted-foreground"
        >
  ...
  // Page variant - modern search with hero-style design for agent pages
  return (
      ...
      >
        ...
        <div className="relative">
          ...
          <EnhancedSearchBar
            ...
            recentSearches={[]}
            placeholder={placeholder}
            className="h-12 text-lg bg-background/95 backdrop-blur-sm border-muted-foreground/20 hover:border-muted-foreground/30 focus:border-primary transition-colors shadow-lg"
            autoFocus={autoFocus}
            showSuggestions={showSuggestions}
          />
        </div>
